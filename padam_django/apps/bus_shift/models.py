...
class BusStop(models.Model):
    ...
    place = models.OneToOneField(
        "geography.Place", on_delete=models.CASCADE, null=False
    )  # un arret pour un point geographique
    stop = models.DateTimeField(blank=True, default="", null=True)

    def __str__(self):
        return (
            f"Stop: {self.place.name} - {self.stop.hour}"
            if self.stop
            else f"Stop: {self.place.name}"
        )
...
class BusShift(models.Model):
    ...
    bus = models.OneToOneField(
        "fleet.Bus", on_delete=models.CASCADE, null=True, blank=True
    )  # un bus par trajet
    driver = models.OneToOneField(
        "users.User", on_delete=models.CASCADE, null=False
    )  # un chauffeur par bus par trajet
    ...
    def time_total(self):
        ...
        return (
            (self.heure_arrive - self.heure_depart)
            if self.heure_arrive and self.heure_depart
            else "-"
        )
    ...
    def heure_depart(self):
        ...
        """
        bus_stop_first = (
            BusStopInBusShift.objects.filter(bus_shift=self.pk)
            .order_by("order")
            .first()
        )
    ...
    def heure_arrive(self):
        ...
        """
        bus_stop_last = (
            BusStopInBusShift.objects.filter(bus_shift=self.pk).order_by("order").last()
        )
