...
def validate_block_bus_stop(cleaned_data) -> dict:
    ...
    if cleaned_data.get("order"):
        stops = BusStopInBusShift.objects.filter(bus_shift=cleaned_data.get("bus_shift")).order_by("order")
        if stops:
            order = stops.values_list("order", flat=True)
            if cleaned_data.get("order") == 0:
                errors["order"] = "il est obligatoire de remplir l'order de passage des arrets (ne pas mettre 0)"
            elif cleaned_data.get("order") in order:
                if cleaned_data.get("bus_stop").pk != BusStopInBusShift.objects.values_list("bus_stop__id",
                                                                                            flat=True).get(
    ...
    if cleaned_data["date"]:
        stops = BusStopInBusShift.objects.filter(bus_shift=cleaned_data.get("bus_shift")).order_by("date")
        if stops:
            if cleaned_data["order"] != 1:
                if stops.filter(order=cleaned_data["order"] - 1).exists():
                    if cleaned_data["date"] < stops.get(order=cleaned_data["order"] - 1).date:
        ...
        if bus_non_dispos :
            for bus in bus_non_dispos:
                ...
                if bus.heure_depart <= cleaned_data["date"] <= bus.heure_arrive and (bus_driver.pk != bus.pk):
        ...
        if driver_non_dispos:
            for driver in driver_non_dispos:
                if driver.heure_depart <= cleaned_data["date"] <= driver.heure_arrive and (bus_driver.pk != driver.pk):
